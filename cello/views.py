...
def get_UI_stream(stream_id, canEdit, filter_tag, sort_order, search_term):
    ...
    for i in stream_items:
        ...
        blockstext = ""
        dbblocks = model.ItemRelationship.select().where((model.ItemRelationship.leftItem == i.id) & (model.ItemRelationship.relationshipType == model.RelationshipType.BLOCKS))  
        for b in dbblocks:
            blockee_id = b.rightItem
            blockee = model.Item.get(model.Item.id == blockee_id)
...
def get_potential_parents():   
    ...
    stream = model.Stream.get(model.Stream.id == stream_id)
    board = model.Board.get(model.Board.id == stream.parentboard)
    board_stream_ids = model.Stream.select(fn.Distinct(model.Stream.id)).where((model.Stream.parentboard==board.id) & (model.Stream.stream_type != model.StreamType.COMPLETED))
    
    parr =  []
    potentials = model.Item.select().where((model.Item.parentstream << board_stream_ids) & (model.Item.id != item_id)).order_by(model.Item.featureId)
    for p in potentials:
        id = p.id
